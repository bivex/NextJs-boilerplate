...
// Force edge runtime to prevent static generation issues with error testing
...
const RenderErrorComponent = () => {
  throw new Error(
    'Render Error: This component always throws an error during render'
  );
};
    ...
    try {
      await new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error('Async Error: This error occurs in an async operation')
            ),
          1000
        )
      );
...
const CustomErrorsComponent = ({
  ...
  onErrorLog: (_type: string, _message: string, _component: string) => void;
}) => {
...
export default function TestErrorPage() {
  ...
  const addErrorLog = (_type: string, _message: string, _component: string) => {
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600">
            This page contains various error scenarios to test Spotlight error
            monitoring. Click the buttons below to trigger different types of
            errors.
          </p>
        </div>
        ...
        {errorLogs.length > 0 && (
          <div className="mb-8 rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold">Recent Error Log</h2>
            <div className="max-h-60 space-y-2 overflow-y-auto">
              {errorLogs.map((log, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-3 rounded-md border border-red-200 bg-red-50 p-3"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-red-700">{log._message}</p>
                    <p className="text-xs text-red-500">{log._component}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-4 flex items-center justify-between">
              <p className="text-sm text-gray-600">
                Showing last {errorLogs.length} error
                {errorLogs.length !== 1 ? 's' : ''}
              </p>
              <Button
                onClick={() => {
                  setErrorLogs([]);
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold">Basic Error Types</h2>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-4">
          ...
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold">Event & Async Errors</h2>
            <div className="space-y-4">
              <div>
          ...
          {/* Safe Execute Demo */}
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold">Safe Execute Demo</h2>
          ...
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold">Manual Error Trigger</h2>
            <p className="mb-4 text-gray-600">
              Click this button to manually throw an error in the console and
              test Spotlight capture:
            </p>
            <div className="space-y-4">
              <Button
                onClick={() => {
                  const error = new Error(
                    'Manual Error: User triggered error for testing Spotlight'
                  );
                  logError(error, 'TestErrorPage.manualTrigger');
                  addErrorLog(
                    'Manual Error',
                    'User triggered error for testing Spotlight',
                    'TestErrorPage'
                  );
                  throw error;
                }}
              ...
              {lastError && (
                <div className="rounded-md border border-red-200 bg-red-50 p-4">
                  <h4 className="mb-2 text-sm font-medium text-red-800">
                    Last Error Triggered:
                  </h4>
                  <div className="text-sm text-red-700">
                    <p>
                      <strong>Type:</strong> {lastError._type}
                    </p>
                    <p>
                      <strong>Message:</strong> {lastError._message}
                    </p>
                    <p>
                      <strong>Component:</strong> {lastError._component}
                    </p>
                    <p>
                      <strong>Time:</strong>{' '}
                ...
                onClick={() => {
                  try {
                    ...
                    addErrorLog(
                      ...
                      'Attempted to call method on null object',
                      'JavaScript Runtime'
                    );
                    throw error;
                  }
                ...
                onClick={() => {
                  try {
                    ...
                    addErrorLog(
                      ...
                      'Attempted to access undefined array element',
                      'JavaScript Runtime'
                    );
                    throw error;
                  }
              ...
              <Button
                onClick={() => {
                  try {
                    ...
                    addErrorLog(
                      'JSON Parse Error',
                      'Failed to parse invalid JSON string',
                      'JavaScript Runtime'
                    );
                    throw error;
                  }
                ...
                onClick={() => {
                  try {
                    ...
                    addErrorLog(
                      ...
                      'Error thrown from dynamically created function',
                      'JavaScript Runtime'
                    );
                    throw error;
                  }
                ...
                <Button
                  onClick={async () => {
                    try {
                      ...
                      logError(
                        error as Error,
                        'TestErrorPage.connectionRefused'
                      );
                      addErrorLog(
                        'Connection Refused',
                        'Failed to connect to server',
        ...
        <div className="mt-12 text-center text-gray-500">
          <p className="text-sm">
            This page is designed for testing Spotlight error monitoring. All
            errors are logged and should appear in your Spotlight dashboard.
          </p>
        </div>
      </div>
    </div>
